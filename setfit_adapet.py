"""""
SCRIPT from https://github.com/huggingface/setfit/blob/main/scripts/adapet/ADAPET/setfit_adapet.py
(minimal modificatons)
"""""
...
def create_fewshot_splits(sample_size, dataset: Dataset) -> DatasetDict:
    ...
    if sample_size == 500:
        ...
        for idx, seed in enumerate(range(0,1)):
            ...
            splits_ds[f"train-{sample_size}-{seed}"] = Dataset.from_pandas(split_df, preserve_index=False)
...
def make_pattern(task_name, parser, english, prompt, lang_pattern):
    ...
    elif task_name in GLUE_DATASETS:
...
def json_file_setup(task_name, train_ds, lang_pattern, max_tokens, parser):
    args = parser.parse_args()
    if args.dict_verbalizer is not  None:
        verbalizer = args.dict_verbalizer
    else:
...
def main(parser):
    ...
    for sample_size in SAMPLE_SIZES:
        ...
        for ds_seed, ds in enumerate(fewshot_ds):                  
            ...
            print(updated_args.pattern)
            print(updated_args.dict_verbalizer) 
            # call ADAPET
