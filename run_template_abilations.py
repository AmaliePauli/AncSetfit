'''
CREDITS
Modifying from: https://github.com/huggingface/setfit/tree/main/scripts/setfit
'''
...
from templatesAblations import DEV_DATASET_TO_METRIC, DEV_LABELS, DEV_LABELS_A,DEV_TEMPLATES, DEV_TEMPLATES_A, DEV_TEMPLATES_Q
...
def parse_args():
    ...
    parser.add_argument("--setting_name", type=str, default='PEM')
...
class SetFitBaseModel:
    def __init__(self, model, max_seq_length: int, add_normalization_layer: bool) -> None:
        self.model = SentenceTransformer(model, device='cuda:1')
...
def load_data_splits(
    dataset: str, sample_sizes: List[int]
...
class RunFewShot:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.output_path = (
            parent_directory
            / "results"
            / f"TEM-{args.model.replace('/', '-')}-{args.loss}-{args.classifier}-itarations_{args.num_itaration}-batch_{args.batch_size}-{args.exp_name}--margin_{args.margin}--{args.setting_name}".rstrip(
                "-"
            )
        )
    ...
    def get_permutated_labels(self, labels):
        permuatations=list(itertools.permutations(labels))
        no_same_spot = np.array(permuatations)[np.sum(np.array(permuatations)==np.array(labels),axis=1)==0]
        permu_labels=random.choice(no_same_spot)
        return permu_labels
    ...
    def train_eval_all_datasets(self) -> None:
        ...
        for dataset, metric in self.dataset_to_metric.items():
            ...
            if self.args.setting_name=='PEM':
                label_names=self.get_permutated_labels(DEV_LABELS[dataset])
                template=DEV_TEMPLATES[dataset]
            elif self.args.setting_name=='ALF':
                    alfabet=list(map(chr, range(97, 123)))
                    label_names=alfabet[0:len(DEV_LABELS[dataset])]
                    template=DEV_TEMPLATES[dataset]
            elif self.args.setting_name=='SHORT':
                label_names=DEV_LABELS[dataset]
                template=''
            elif self.args.setting_name=='A':
                label_names=DEV_LABELS_A[dataset]
                template=DEV_TEMPLATES_A[dataset]
            elif self.args.setting_name=='B':
                label_names=DEV_LABELS_B[dataset]
                template=DEV_TEMPLATES_B[dataset]
            elif self.args.setting_name=='QA':
                label_names=DEV_LABELS[dataset]
                template=DEV_TEMPLATES_Q[dataset]
            else:  
                label_names=DEV_LABELS[dataset]
                template=DEV_TEMPLATES[dataset]
